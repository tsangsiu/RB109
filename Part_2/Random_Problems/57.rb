...
ls (list of distances, all distances are positive or zero integers and this list
...
def choose_best_sum(max_dist, n_town, list)
  return nil if list.size < n_town
  combinations = list.combination(n_town).to_a
  combinations_under_max_dist = combinations.select { |combination| combination.sum <= max_dist }
  return nil if combinations_under_max_dist.empty?
  combinations_max_dist = combinations_under_max_dist.max_by { |combination| combination.sum }
  combinations_max_dist.sum
end

...
- If all elements in `arr2` is a square number of a number in `arr1`, return true
- Return false if otherwise

# Algorithm
- Return false if either `arr1` or `arr2` is nil
- From `arr1`, create a new array where each number is squared
- Sort the result array and `arr2`
- Return true if the above two arrays are the same, return false if otherwise

=end

def comp(arr1, arr2)
  return false if arr1.nil? || arr2.nil?
  arr1_sq = arr1.map { |num| num ** 2 }
  arr1_sq.sort == arr2.sort
end
